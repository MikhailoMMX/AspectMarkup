      ...
      /// <param name="size"></param>
        ...
        public bool Parse()
        {
            ...
            while (true)
            {
                ...
                {
                    //MikhailoMMX: EOF recovery
                    if (NextToken == endOfFileToken)
                        return TryBuildTreeFromStack(false);
                    if (!TryBuildTreeFromStack(true))
                        return false;
                    if (!ErrorRecovery())
                        return false;
...
#pragma warning restore 649
    ...
    internal class State
    {
      ...
      /// </summary>
        ...
        internal List<KeyValuePair<int, int>> kernelItems = new List<KeyValuePair<int, int>>();
        ...
        public State(int[] actions, int[] goToList, int[] kernelItems)
        {
            ...
            for (int i = 0; i < kernelItems.Length; i += 2)
                this.kernelItems.Add(new KeyValuePair<int, int>(kernelItems[i], kernelItems[i+1]));
        }
        ...
        public State(int defaultAction, int[] goToList, int[] kernelItems)
        {
            this.defaultAction = defaultAction;
