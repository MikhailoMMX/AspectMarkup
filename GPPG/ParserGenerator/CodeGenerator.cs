    ...
    internal class CodeGenerator
    {
        ...
        private static void GenerateShiftReduceMachineState( int stateNumber, AutomatonState state ) {
            ...
            //if (state.nonTerminalTransitions.Count > 0) {
                ...
                Console.Write( '}' );
            //}
            //MikhailoMMX: Generate Kernel Items info
            Console.Write(", new int[]{");
            List<int> L = new List<int>();
            foreach (ProductionItem p in state.kernelItems)
            {
                if (p.pos == 1 && p.production.lhs == p.production.rhs[0])
                    continue;
                L.Add(p.production.num);
                L.Add(p.pos);
            }
        ...
        private void GenerateShiftReduceMachineActions( List<Production> productions ) {
            ...
            Console.WriteLine( "  {" );
            Console.WriteLine("    CurrentSemanticValue = new ValueType();");
